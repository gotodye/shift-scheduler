...
const ref = doc(db, “shifts”, date + “**” + personId);
...
deleteShift(date, personId){ return deleteDoc(doc(db, “shifts”, date + “**” + personId)).catch(()=>{}); },
...
// 等 app.js 把對外 hook（setAccess / UNIT_IDS 等）掛好，避免載入時序造成
// “window.setAccess is not a function”。最多等 ~5 秒後放行讓後續照常處理。
function whenAppReady(){
return new Promise(resolve => {
const ok = () => typeof window.setAccess === ‘function’ && Array.isArray(window.UNIT_IDS);
if(ok()) return resolve();
let n = 0;
const iv = setInterval(() => {
if(ok() || ++n > 100){ clearInterval(iv); resolve(); }
}, 50);
});
}
...
try{
if(typeof window.setAccess !== ‘function’){ throw new Error(‘app.js 尚未載入完成（setAccess 未定義）’); }
