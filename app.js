...
if(!window.SB){ alert(‘SB 未載入’); return; }
btn.disabled = true;
try{
await window.SB.writeUser(email, { admin, viewer, units, name:’’ });
$(’#umEmail’).value=’’; $(’#umAdmin’).checked=false; $(’#umViewer’).checked=false;
$$(’#umUnits input’).forEach(i=>i.checked=false);
renderUserMgmt();                       // 立即重繪，不依賴雲端快照回來
}catch(e){
alert(t(‘um_save_fail’) + (e && e.message ? e.message : e));
}finally{
btn.disabled = false;
}
};
